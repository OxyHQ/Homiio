...
export const AMENITIES: Amenity[] = [
    ...
    // Technology - Essential for modern life
    {
        ...
        ethicalNotes: 'High-speed internet supports work, education, and participation in digital society',
        environmental: 'neutral'
    },
    {
        id: 'cable_tv',
    ...
    // Pet-Friendly - Reasonable accommodation for animal companions
    {
        ...
        ethicalNotes: 'Pet policies should be reasonable and not discriminatory. Service animals must be accommodated without fees.',
        environmental: 'neutral'
    },
    {
        id: 'dog_park',
...
export const calculateEthicalAmenityValue = (amenityIds: string[], baseRent: number): {
    totalValue: number;
    warnings: string[];
    ...
    const totalValue = selectedAmenities.reduce((sum, amenity) => sum + (amenity.maxFairValue || 0), 0);
    ...
    if (percentageOfRent > ETHICAL_AMENITY_GUIDELINES.maxValueAddPercentage) {
        warnings.push(`Amenity value (${percentageOfRent.toFixed(1)}%) exceeds ethical limit of ${ETHICAL_AMENITY_GUIDELINES.maxValueAddPercentage}%`);
    }

    if (totalValue > ETHICAL_AMENITY_GUIDELINES.maxTotalAmenityValue) {
        warnings.push(`Total amenity value ($${totalValue}) exceeds ethical limit of $${ETHICAL_AMENITY_GUIDELINES.maxTotalAmenityValue}`);
    }

    // Check for accessibility violations
    const accessibilityAmenities = selectedAmenities.filter(a => a.accessibility);
    const accessibilityValue = accessibilityAmenities.reduce((sum, amenity) => sum + (amenity.maxFairValue || 0), 0);
    if (accessibilityValue > 0) {
        warnings.push('Accessibility features should never add to rent - this may violate fair housing laws');
    }

    // Check for essential amenity overpricing
    const essentialAmenities = selectedAmenities.filter(a => a.essential);
    const essentialValue = essentialAmenities.reduce((sum, amenity) => sum + (amenity.maxFairValue || 0), 0);
    if (essentialValue > ETHICAL_AMENITY_GUIDELINES.essentialAmenitiesMaxValue) {
        warnings.push(`Essential amenities are overpriced ($${essentialValue} vs max $${ETHICAL_AMENITY_GUIDELINES.essentialAmenitiesMaxValue})`);
    }

    // Determine ethical score
    let ethicalScore: 'excellent' | 'good' | 'fair' | 'concerning';
    if (warnings.length === 0 && totalValue <= ETHICAL_AMENITY_GUIDELINES.maxTotalAmenityValue * 0.6) {
...
export const checkFairHousingCompliance = (amenityIds: string[]): {
    compliant: boolean;
    violations: string[];
