 ...
 * `HomeSearch` composer and the `SearchResultsView` exchange. They are pure
 ...
 * truth the results view reads and the `HomeSearch` composer writes. Every field is
 ...
 * sidebar toggle, and the `HomeSearch` composer can all share one mapping.
 */
 ...
 * `HomeSearch` sheet's toggle, whose draft stores the active offering rather than a
 * browse mode.
 */
