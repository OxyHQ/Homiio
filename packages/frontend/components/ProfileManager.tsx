...
export function ProfileManager() {
  ...
  const handleDeleteProfile = async (profileId: string) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this profile?', [
    ...
    <View style={styles.container}>
      ...
      <View style={styles.selector}>
        <TouchableOpacity
          style={[
            ...
            selectedProfileType === ProfileType.PERSONAL && styles.selectorButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              ...
              selectedProfileType === ProfileType.PERSONAL && styles.selectorButtonTextActive,
            ]}
          >
            Personal
          </Text>
        ...
        <TouchableOpacity
          style={[
            styles.selectorButton,
            selectedProfileType === ProfileType.AGENCY && styles.selectorButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              styles.selectorButtonText,
              selectedProfileType === ProfileType.AGENCY && styles.selectorButtonTextActive,
            ]}
          >
            Agency
          </Text>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.statusText}>
          Primary Profile:{' '}
          {primaryProfile
            ? `${primaryProfile.profileType} (${primaryProfile.isPrimary ? 'Primary' : 'Secondary'})`
            : 'None'}
        </Text>
        <Text style={styles.statusText}>Total Profiles: {allProfiles.length}</Text>
        <Text style={styles.statusText}>Personal: {personalProfile ? '✓' : '✗'}</Text>
        <Text style={styles.statusText}>Agency: {agencyProfiles.length}</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        {selectedProfileType === 'personal' && !personalProfile && (
          ...
          >
            <Text style={styles.buttonText}>Create Personal Profile</Text>
          </TouchableOpacity>
        )}
        ...
        {selectedProfileType === 'agency' && (
          ...
          >
            <Text style={styles.buttonText}>Create Agency Profile</Text>
          </TouchableOpacity>
        )}
        ...
        {((selectedProfileType === 'personal' && personalProfile) ||
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              const profile =
                selectedProfileType === 'personal' ? personalProfile : agencyProfiles[0];
          ...
          >
            <Text style={styles.buttonText}>Toggle Profile Active Status</Text>
          </TouchableOpacity>
        )}
      ...
      <View style={styles.section}>
        ...
        {selectedProfileType === 'personal' && personalProfile && (
          <View style={styles.profileDetails}>
            <Text style={styles.detailText}>
              Trust Score: {personalProfile.personalProfile?.trustScore.score || 0}
            </Text>
            <Text style={styles.detailText}>
              Verified: {personalProfile.personalProfile?.verification.identity ? '✓' : '✗'}
            </Text>
            <Text style={styles.detailText}>
              Max Rent: ${personalProfile.personalProfile?.preferences.maxRent || 'Not set'}
            </Text>
          </View>
        )}
        ...
        {selectedProfileType === 'agency' && agencyProfiles.length > 0 && (
          <View style={styles.profileDetails}>
            <Text style={styles.detailText}>
              Business Type: {agencyProfiles[0].agencyProfile?.businessType || 'Not set'}
            </Text>
            <Text style={styles.detailText}>
              Members: {agencyProfiles[0].agencyProfile?.members.length || 0}
            </Text>
            <Text style={styles.detailText}>
              Rating: {agencyProfiles[0].agencyProfile?.ratings.average || 0}/5
            </Text>
          </View>
        )}
      </View>
