...
import type { LeasePaymentStatus, LeaseStatus } from '@homiio/shared-types';
...
/** Lease payment status → hue + label (keeps the labels the old StatusBadge mapping showed). */
const PAYMENT_STATUS_MAP: Record<LeasePaymentStatus, StatusEntry> = {
  paid: { hue: 'lime', i18nKey: 'statusBadge.completed' },
  overdue: { hue: 'rose', i18nKey: 'statusBadge.error' },
  cancelled: { hue: 'neutral', i18nKey: 'statusBadge.cancelled' },
  pending: { hue: 'yellow', i18nKey: 'statusBadge.pending' },
};
...
export function LeasePaymentStatusBadge({ status }: { status: LeasePaymentStatus }) {
  const { t } = useTranslation();
  const entry = PAYMENT_STATUS_MAP[status] ?? UNKNOWN;
  return (
    <Chip size="small" hue={entry.hue}>
      {t(entry.i18nKey)}
    </Chip>
