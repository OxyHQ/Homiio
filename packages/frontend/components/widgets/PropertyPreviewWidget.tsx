...
export function PropertyPreviewWidget() {
    ...
    const sections: PreviewSection[] = useMemo(() => [
        {
            ...
            isComplete: !!(formData?.basicInfo?.propertyType && formData?.basicInfo?.bedrooms !== undefined),
            hasData: !!(formData?.basicInfo?.propertyType || formData?.basicInfo?.bedrooms !== undefined)
        },
        {
            id: 'location',
            title: 'Location',
        ...
        },
        {
            ...
            title: 'Amenities',
            icon: 'list-outline',
            isComplete: !!(formData?.amenities?.selectedAmenities && formData?.amenities?.selectedAmenities.length > 0),
        ...
        },
        {
            ...
            isComplete: !!(formData?.basicInfo?.description && formData?.basicInfo?.description.length > 10),
    ...
    const amenitiesDisplay = useMemo(() => {
        if (!formData?.amenities?.selectedAmenities || formData.amenities.selectedAmenities.length === 0) return [];
            ...
            >
                <View style={styles.emptyContainer}>
                    ...
                    <ThemedText style={styles.emptySubtitle}>
                        Fill out the form to see a live preview of how your property will appear to potential tenants
                    </ThemedText>
                </View>
            </BaseWidget>
        );
            ...
            >
                        ...
                        {formData.media?.images && formData.media.images.length > 0 ? (
                            ...
                            >
                                ...
                                <ThemedText style={styles.imageCountText}>
                                    {formData.media.images.length} photo{formData.media.images.length !== 1 ? 's' : ''}
                                </ThemedText>
                            </LinearGradient>
                        ) : (
                                    ...
                                    <View style={styles.sectionData}>
                                        ...
                                        {formData.basicInfo?.bedrooms !== undefined && (
                                            <View style={styles.dataItem}>
                                                ...
                                                <ThemedText style={styles.dataValue}>{formData.basicInfo.bedrooms}</ThemedText>
                                            </View>
                                        )}
                                        {formData.basicInfo?.bathrooms !== undefined && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Bathrooms:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{formData.basicInfo.bathrooms}</ThemedText>
                                            </View>
                                        )}
                                        {formData.basicInfo?.squareFootage && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Square Footage:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{formData.basicInfo.squareFootage} sqft</ThemedText>
                                            </View>
                                        )}
                                    </View>
                                )}
                                    ...
                                    <View style={styles.sectionData}>
                                        ...
                                        {formData.location?.zipCode && (
                                            <View style={styles.dataItem}>
                                                ...
                                                <ThemedText style={styles.dataValue}>{formData.location.zipCode}</ThemedText>
                                            </View>
                                        )}
                                    </View>
                                )}
                                    ...
                                    <View style={styles.sectionData}>
                                        {formData.pricing?.monthlyRent && formData.pricing.monthlyRent > 0 && (
                                            <View style={styles.dataItem}>
                                                ...
                                                <ThemedText style={styles.dataValue}>${formData.pricing.monthlyRent.toLocaleString()}</ThemedText>
                                            </View>
                                        )}
                                        {formData.pricing?.securityDeposit && formData.pricing.securityDeposit > 0 && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Security Deposit:</ThemedText>
                                                <ThemedText style={styles.dataValue}>${formData.pricing.securityDeposit.toLocaleString()}</ThemedText>
                                            </View>
                                        )}
                                        {formData.pricing?.currency && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Currency:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{formData.pricing.currency}</ThemedText>
                                            </View>
                                        )}
                                        {formData.pricing?.includedUtilities && formData.pricing.includedUtilities.length > 0 && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Included Utilities:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{formData.pricing.includedUtilities.join(', ')}</ThemedText>
                                            </View>
                                        )}
                                        {formData.pricing?.applicationFee && formData.pricing.applicationFee > 0 && (
                                            <View style={styles.dataItem}>
                                                ...
                                                <ThemedText style={styles.dataValue}>${formData.pricing.applicationFee.toLocaleString()}</ThemedText>
                                            </View>
                                        )}
                                        {formData.pricing?.lateFee && formData.pricing.lateFee > 0 && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Late Fee:</ThemedText>
                                                <ThemedText style={styles.dataValue}>${formData.pricing.lateFee.toLocaleString()}</ThemedText>
                                            </View>
                                        )}
                                    </View>
                                )}
                                    ...
                                    <View style={styles.sectionData}>
                                        {amenitiesDisplay.length > 0 && (
                                            <View style={styles.amenitiesGrid}>
                                                ...
                                                {formData.amenities?.selectedAmenities && formData.amenities.selectedAmenities.length > 6 && (
                                        ...
                                        {formData.rules?.petsAllowed !== undefined && (
                                            <View style={styles.dataItem}>
                                                ...
                                                <ThemedText style={styles.dataValue}>{yesNo(formData.rules.petsAllowed)}</ThemedText>
                                            </View>
                                        )}
                                        {formData.rules?.smokingAllowed !== undefined && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Smoking Allowed:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{yesNo(formData.rules.smokingAllowed)}</ThemedText>
                                            </View>
                                        )}
                                        {formData.rules?.partiesAllowed !== undefined && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Parties Allowed:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{yesNo(formData.rules.partiesAllowed)}</ThemedText>
                                            </View>
                                        )}
                                        {formData.rules?.guestsAllowed !== undefined && (
                                            <View style={styles.dataItem}>
                                                <ThemedText style={styles.dataLabel}>Guests Allowed:</ThemedText>
                                                <ThemedText style={styles.dataValue}>{yesNo(formData.rules.guestsAllowed)}</ThemedText>
                                            </View>
                                        )}
                                    </View>
                                )}
                ...
                <View style={styles.publishStatusContainer}>
                    <View style={[
                        ...
                        completionPercentage >= 80 ? styles.publishStatusReady : styles.publishStatusIncomplete
                    ]}>
                        <IconComponent
                        ...
                        <ThemedText style={[
                            ...
                            {completionPercentage >= 80 ? 'Ready to Publish' : `${100 - completionPercentage}% more to complete`}
                        </ThemedText>
                    </View>
                </View>
            </ScrollView>
