...
interface SavedPropertiesContextType {
    ...
    createFolder: (folderData: { name: string; description?: string; color?: string; icon?: string }) => Promise<SavedPropertyFolder | undefined>;
    updateFolder: (folderId: string, folderData: { name?: string; description?: string; color?: string; icon?: string }) => Promise<SavedPropertyFolder | undefined>;
...
export const SavedPropertiesProvider: React.FC<SavedPropertiesProviderProps> = ({ children }) => {
    ...
    const createFolder = useCallback(async (folderData: { name: string; description?: string; color?: string; icon?: string }) => {
        if (!oxyServices || !activeSessionId) return;
    ...
    const updateFolder = useCallback(async (folderId: string, folderData: { name?: string; description?: string; color?: string; icon?: string }) => {
        if (!oxyServices || !activeSessionId) return;
    ...
    const savePropertyToFolder = useCallback(async (propertyId: string, folderId: string | null) => {
        if (!oxyServices || !activeSessionId) return;
