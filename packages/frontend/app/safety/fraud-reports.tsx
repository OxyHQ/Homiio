...
const sampleReports: FraudReport[] = [
  {
    ...
    description:
      'Landlord requested payment outside the platform. They asked for a direct bank transfer to an unverified account.',
    severity: 'high',
    date: '2023-05-10',
  ...
  },
  {
    ...
    description:
      'Property does not exist at the specified address. Appears to be a scam listing with stolen photos.',
    severity: 'critical',
    date: '2023-05-05',
    status: 'active',
  ...
  },
  {
    ...
    description:
      'Unusual activity detected during the verification process. Multiple failed attempts with different documents.',
    severity: 'medium',
    date: '2023-05-08',
    status: 'investigating',
    reportedBy: 'System',
  ...
  },
  {
    ...
    description:
      'Property amenities and conditions significantly different from listing description.',
    severity: 'low',
    date: '2023-04-28',
    status: 'resolved',
...
export default function FraudReportsScreen() {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.safetyInfoCard}>
        ...
        <View style={styles.safetyInfoContent}>
          ...
          <Text style={styles.safetyInfoDescription}>
            {t(
              'We take fraud seriously. Always pay through our platform and verify property details before signing contracts.',
            )}
          </Text>
          <TouchableOpacity
            ...
            <View style={styles.statsContainer}>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>
                  {sampleReports.filter((r) => r.status === 'active').length}
                </Text>
                <Text style={styles.statLabel}>{t('Active')}</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>
                  {sampleReports.filter((r) => r.status === 'investigating').length}
                </Text>
                <Text style={styles.statLabel}>{t('Investigating')}</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>
                  {sampleReports.filter((r) => r.status === 'resolved').length}
                </Text>
                    ...
                    <View style={styles.infoActionsRow}>
                      ...
                      {report.status !== 'resolved' && (
                        ...
                        >
                          ...
                          />
                          <Text
                            style={[styles.infoActionText, { color: customColors.COLOR_GREEN }]}
                          >
                            {t('Mark as Resolved')}
                          </Text>
                        </TouchableOpacity>
      ...
      <TouchableOpacity style={styles.reportFraudButton} onPress={handleReportFraud}>
