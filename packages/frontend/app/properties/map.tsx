...
export default function PropertiesMapScreen() {
  ...
  const mapProperties: MapProperty[] = properties
    .filter(
      (property) => property.location?.coordinates && property.location.coordinates.length === 2,
    )
    .map((property) => ({
      ...property,
        ...
        <View style={styles.selectedPropertyInfo}>
          <View style={styles.selectedPropertyContent}>
            ...
            <Text style={styles.selectedPropertyPrice}>
              ${selectedProperty.rent.amount}/
              {selectedProperty.priceUnit || selectedProperty.rent.paymentFrequency}
            </Text>
          </View>
          <TouchableOpacity
      ...
      {__DEV__ && (
        <View style={styles.debugContainer}>
          <Text style={styles.debugText}>
            Properties: {properties.length} | Map Center:{' '}
            {mapCenter ? `${mapCenter.lat.toFixed(4)}, ${mapCenter.lng.toFixed(4)}` : 'Not set'} |
