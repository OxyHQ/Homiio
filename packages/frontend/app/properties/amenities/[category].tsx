...
export default function AmenitiesByCategoryPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      essentialCount,
      averageValue:
        categoryAmenities.length > 0 ? Math.round(totalValue / categoryAmenities.length) : 0,
    };
  }, [categoryAmenities]);
  ...
  const renderAmenityCard = (amenity: any) => (
    <TouchableOpacity
      key={amenity.id}
      style={[styles.amenityCard, amenity.premium && styles.premiumAmenityCard]}
      onPress={() => {
    ...
    >
      <View style={styles.amenityCardHeader}>
        ...
        <View style={styles.amenityInfo}>
          <Text style={[styles.amenityName, amenity.premium && styles.premiumAmenityName]}>
            {amenity.name}
            {amenity.premium && <Text style={styles.premiumTag}> PREMIUM</Text>}
          </Text>
          {amenity.valueAdd && <Text style={styles.amenityValue}>+${amenity.valueAdd}/month</Text>}
        </View>
      </View>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={[styles.categoryHeader, { backgroundColor: categoryInfo.color + '10' }]}>
          <View style={[styles.categoryHeaderIcon, { backgroundColor: categoryInfo.color + '20' }]}>
            <IconComponent name={categoryInfo.icon} size={32} color={categoryInfo.color} />
          </View>
        ...
        <View style={styles.controlsSection}>
          ...
          <View style={styles.sortContainer}>
            ...
            >
              <Text
                style={[styles.sortButtonText, sortBy === 'value' && styles.activeSortButtonText]}
              >
                Value
              </Text>
            </TouchableOpacity>
        ...
        <View style={styles.infoSection}>
          ...
          <Text style={styles.infoText}>
            The values shown represent the estimated monthly rental value each amenity adds to a
            property. These are based on market research and help landlords price their properties
            fairly while giving tenants insight into what they&apos;re paying for.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
