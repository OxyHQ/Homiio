...
export function useAddressDetail(): UseAddressDetailReturn {
  ...
  const geocodeAddress = useCallback(async (address: string): Promise<AddressCoordinates | null> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const fetchNeighborhoodData = useCallback(async (coordinates: AddressCoordinates): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const fetchNearbyAmenities = useCallback(async (coordinates: AddressCoordinates, radius: number = 1): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      const mockAmenities = [
        { name: 'Whole Foods Market', type: 'grocery' as const, distance: 0.3, rating: 4.5, address: '123 Main St', phone: '+1-555-0123' },
        { name: 'Starbucks', type: 'restaurant' as const, distance: 0.2, rating: 4.2, address: '456 Oak Ave', phone: '+1-555-0124' },
        { name: 'CVS Pharmacy', type: 'pharmacy' as const, distance: 0.4, rating: 4.0, address: '789 Pine St', phone: '+1-555-0125' },
        { name: 'Central Park', type: 'park' as const, distance: 0.8, rating: 4.8, address: 'Central Park', phone: '+1-555-0126' },
        { name: 'Metro Station', type: 'transit' as const, distance: 0.5, rating: 4.3, address: 'Subway Station', phone: '+1-555-0127' },
        { name: 'Public Library', type: 'school' as const, distance: 0.6, rating: 4.4, address: '321 Library Blvd', phone: '+1-555-0128' },
        { name: 'Shopping Center', type: 'shopping' as const, distance: 0.7, rating: 4.1, address: '654 Mall Dr', phone: '+1-555-0129' },
        { name: 'Local Hospital', type: 'hospital' as const, distance: 1.2, rating: 4.6, address: '987 Medical Center', phone: '+1-555-0130' }
      ];
