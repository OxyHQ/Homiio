 ...
 *
 * Answer quickly and queue the processing. So exactly two writes happen here, in
 * ONE transaction — the event's audit row is completed and a durable
 ...
 * — a decision silently lost, with a row saying it arrived. Committing both
 * together means the only two outcomes are "recorded and queued" or "neither",
...
export interface RecordDecisionEventInput {
   ...
   * contract by the worker that acts on it — these payloads are loose by design,
   * and validating here would mean an event whose shape this deployment does not
   * recognise yet is refused at the door and retried until it expired, instead of
   * being kept until the code catches up.
   */
  decision: unknown;
}
