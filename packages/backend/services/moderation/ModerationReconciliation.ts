 ...
 *    was dropped by the retention TTL while the deployment was down for longer
 ...
 *    subject provider (an eviction case), or because it predates the
 *    integration. This is the one divergence that is not a fault, so it is
 *    counted and NEVER re-queued — re-deriving a delivery event for a report
 *    nothing can describe would send it straight to the dead-letter queue and
 *    turn a deliberate local-only report into a recurring alert. Counting it is
 *    still worth doing: it is the only number that makes "reports stored here
 *    that no jury will ever see" visible at all, and that is precisely the cost
 *    of accepting them.
 *
