 ...
 * same listing produce ONE case and ONE consequence, so enforcement is keyed on
 * the decision and not on the reports — `ModerationEnforcementService` is called
 * a single time no matter how many reports are updated, and its own unique index
 ...
 * overlap — a sender retries for a day, and a correction can arrive while the
 ...
 * happened to this listing". A state-changing action beats a note for a human,
 * which beats an explicit nothing — and `manual_review` still reaches the
 * report, because a reporter told "nothing happened" when a human is about to
 * look is being told something untrue.
 */
...
export async function applyDecisionOutboxEvent(
   ...
   * decision back on a retry schedule until it expired. Parsing at the point of
   * use means an event this deployment cannot yet read waits in the outbox until
   * it can.
   */
  ...
  if (reports.length === 0) {
     ...
     * case id back to this deployment is still being written, or while a
   ...
   * every report merged into a case is about the same object — the first report
   * names it.
   */
  const [first] = reports;
