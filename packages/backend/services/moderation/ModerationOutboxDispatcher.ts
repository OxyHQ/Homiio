 ...
 * stops claiming new work but lets the event already being handled reach a
 * durable state.
 *
 * It is NOT leader-gated, and that is a property of the claim rather than an
 * oversight. Every event is taken under a lease with an owner check, so the API
 * task and the worker task simply share the work — and a task dying mid-delivery
 * has its lease expire and its event reclaimed, which a single leader would not
 * give us.
 *
 ...
 * switched on; running the loop instead would count attempts against a
 * deployment that has nowhere to send anything and dead-letter the backlog it
...
export class ModerationOutboxDispatcher {
  ...
  start(): void {
    ...
    if (!config.crowdSource.enabled) {
      logger.info('[CrowdSource] outbox dispatcher not started', {
        reason: 'CROWDSOURCE_ENABLED=false',
      });
      return;
    }
    this.running = true;
