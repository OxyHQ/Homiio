 ...
 * check that goes red the moment the promise comes due.
 * `ID_COLUMNS_WITHOUT_FOREIGN_KEY` plus `isOxyAccountColumn` are its permanent
 * counterpart, and together with the real constraints they mean EVERY id-shaped
 * column in the schema is classified — which is what lets a NEW one that nobody
 * decided about fail here instead of shipping unconstrained and surfacing as an
 * orphan row three batches later.
 */
...
/**
 * A table that exists only in this file, to exercise `isOxyAccountColumn`
 * against a column whose PROPERTY name and SQL name differ.
 *
 * Migration 0000 contains no Oxy account column at all — the geo tables and
 * `images` have no owner — so without this probe the predicate would be
 * completely unexercised, and a broken one would sail through every assertion
 * below by matching nothing. It is never migrated and never queried; it exists
 * purely to be asked a question.
 */
const probe = pgTable('probe_oxy_account_columns', {
  oxyUserId: text(),
  coverImageId: text(),
});
...
/** Every table the barrel exports. See `schemaInvariants.test.ts` for the widening. */
...
describe('id-shaped column classification', () => {
  it('finds id-shaped columns to classify', () => {
    // Vacuity floor. `idShapedColumns` matches on the SQL name via
    // `sqlColumnName`; matching the TypeScript property name instead would find
    // NOTHING (`coverImageId` does not end in `_id`) and every assertion below
    // would pass trivially over an empty list.
  ...
  it('recognises an Oxy account column by its SQL name, not its property name', () => {
    // The probe's property is `oxyUserId`; its SQL name is `oxy_user_id`. A
    // predicate reading `column.name` would answer false here — and, because no
    // real table in migration 0000 has such a column, nothing else in this file
    // would notice.
    expect(isOxyAccountColumn(probe.oxyUserId)).toBe(true);
    expect(isOxyAccountColumn(probe.coverImageId)).toBe(false);
  });

  it('holds a populated Oxy account name set, all in SQL spelling', () => {
    // Second half of the same vacuity concern: an empty set would make the
    // predicate answer `false` for everything, which is indistinguishable from
    // "no account columns exist yet" until the first one lands and ships
    // unclassified.
    expect(OXY_ACCOUNT_COLUMN_NAMES.size).toBeGreaterThan(5);
    const camelCased = [...OXY_ACCOUNT_COLUMN_NAMES].filter(
  ...
  it('classifies every id-shaped column', async () => {
    ...
    // `ID_COLUMNS_WITHOUT_FOREIGN_KEY` / `DEFERRED_FOREIGN_KEYS` with the reason.
