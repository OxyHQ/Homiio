/**
 * Seed Cities Script
 * Populates the database with initial cities data
 */
...
async function seedCities() {
  try {
    ...
    // Update properties count for each city
    console.log('🔄 Updating properties count for each city...');
    for (const city of cities) {
      await city.updatePropertiesCount();
    }
    console.log('✅ Updated properties count for all cities');
    ...
    const popularCities = await City.find().sort({ propertiesCount: -1 }).limit(5);
