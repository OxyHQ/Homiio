 ...
 * Seed Tips Script
 *
 * Populates the Tips knowledge base (renting guides, safety, legal and
 * inspection articles) served by the `/tips` API. This dataset was previously
 * hardcoded in the tips controller; the controller now reads exclusively from
 * the Tip collection, so run this seed to make the tips screens render.
 *
 * Idempotent: upserts by the canonical `slug` key. Re-running creates no
 * duplicates and updates articles in place.
 *
 * Usage:
 *   bun run seed:tips
 *   # or
 *   ts-node --transpile-only scripts/seedTips.ts
 */
...
interface TipSeed {
  /** Stable human-readable identifier, stored as `articleId`. */
  id: string;
  title: string;
  description: string;
  category: string;
...
/**
 * Upsert every seed tip by its canonical `slug`. Idempotent. Returns the number
 * of articles processed.
 */
export async function seedTips(): Promise<{ tips: number }> {
...
/** CLI entrypoint: connect, seed, disconnect. */
async function run(): Promise<void> {
  console.log('[seed-tips] Connecting to database...');
  await database.connect();

  const summary = await seedTips();
  console.log('[seed-tips] ----------------------------------------');
  console.log(`[seed-tips] Tips: ${summary.tips}`);
  console.log('[seed-tips] Done.');
}
...
if (require.main === module) {
  run()
    .catch((err: unknown) => {
      ...
      console.error('[seed-tips] FAILED:', message);
      process.exitCode = 1;
    })
    .finally(async () => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.warn('[seed-tips] disconnect error:', message);
      }
