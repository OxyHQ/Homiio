...
Secrets for production live in GitHub repo secrets → SSM `/oxy/homiio/*` → ECS task env. Sindi uses Homiio's Oxy service credential; AI provider credentials never enter this repository or task and live only in Kaana's encrypted database. See `~/Oxy/oxy-infra`.
