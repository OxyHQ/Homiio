...
Application and product secrets for production are injected through the exact SSM bindings under `/oxy/homiio/*`; the Oxy service credential is not an AI provider key. Point-inference features call Oxy directly. Interactive Sindi chat calls Alia with that service identity plus the already-verified user id in `X-Oxy-User-Id`, never a human bearer. The agent primary key is fixed to `01a0646a-078f-7514-9800-9f43ceed7df8`, linked 1:1 to bot account `01a0646a-078f-7974-9645-a5e8be237f47` under project account `01a0646a-078f-72ea-8759-86326484a7e0` by Oxy's internal bootstrap. Sindi's fixed prompt and exact `web` grant live in Alia; Homiio sends only trusted request-local property context. Homiio keeps its own conversation records and neither switches into the bot nor gives it autonomous execution. AI provider credentials never enter this repository or task and live only encrypted in Kaana PostgreSQL/KMS. Kaana's sole canonical signed origin is `https://kaana.ai`. See `~/Oxy/oxy-infra`.
