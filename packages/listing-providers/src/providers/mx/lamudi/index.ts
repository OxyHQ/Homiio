/**
 * Lamudi Mexico provider — schema.org JSON-LD + Playwright session warm.
 *
...
import {
  ...
  parseLamudiDetail,
  parseLamudiSearch,
  type LamudiRawListing,
} from './parse';

const PROVIDER_ID: ProviderId = 'lamudi';
...
function searchUrl(city: string, operation: 'for-rent' | 'for-sale', page: number): string {
  const base = `${LAMUDI_BASE_URL}/${citySlug(city)}/${operation}/`;
  return page <= 1 ? base : `${base}?page=${page}`;
}

function asRaw(payload: unknown): LamudiRawListing {
  ...
  if (!record || typeof record.sourceId !== 'string' || typeof record.url !== 'string') {
    throw new Error('lamudi: normalize received an invalid payload');
  }
  return payload as LamudiRawListing;
}
...
export class LamudiProvider implements ListingProvider {
  ...
  constructor(options: LamudiProviderOptions = {}) {
    ...
    for (const city of cities) {
      for (const operation of OPERATIONS) {
        for (let page = 1; page <= MAX_SEARCH_PAGES; page += 1) {
          ...
          const pageUrl = searchUrl(city, operation, page);
          ...
          const refs = parseLamudiSearch(html);
  ...
  private async fetchViaSession(
    ...
    try {
      ...
      return isLamudiChallenge(html) ? undefined : html;
    } catch {
      return undefined;
    } finally {
      await session?.close();
  ...
  async fetch(ref: ExternalListingRef, ctx: FetchContext): Promise<RawListing> {
    ...
    return { ref, payload: parseLamudiDetail(html, ref.url) };
  }
  ...
  async health(): Promise<ProviderHealth> {
    return {
      provider: this.id,
      status: 'degraded',
      detail: `${LAMUDI_BASE_URL} often Cloudflare-blocked — keep OFF until residential discover works`,
    };
  }
}
