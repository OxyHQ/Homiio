...
| `priceCurrency` (one of `LISTING_CURRENCIES`) | `searchQueryBuilder.ts` `PriceBoundPlan`, applied in `search.ts` | **new**: the unit the bound is read in. A bound is narrowed to listings priced in this currency and is **never converted** — no rate Homiio can cite or version exists, and an invented one is an invented price (ADR 0004). An unknown code is dropped, not honoured. When it is absent the server **censuses the resolved scope** (`db/properties/priceCurrency.ts` — the same census the histogram's bars are counted with) and applies the bound in the scope's most common currency; when even that is absent, the bound matches nothing rather than everything. A price whose currency column is null is excluded, the same rule `areaInRange` gives an area stored as `0`. Sent on its own, with no bound, it filters nothing |
...
| `checkIn`, `checkOut` | `searchQueryBuilder.ts`, `propertyFilters.ts` | **new on SEARCH**: a dated search excludes a home the host's calendar blocks, one with a confirmed reservation overlapping, and one committed to a confirmed exchange — the same three predicates `GET /api/properties` has applied since the Postgres port. Both bounds are required and must be in order; a half-open or reversed range narrows nothing. Boundaries are half-open, so back-to-back stays do not conflict |
| `features` (comma list from `HOUSING_FEATURES`) | `searchQueryBuilder.ts`, `propertyFilters.ts#featureColumnCondition` | **new**: Bloom's eleven housing features. Five resolve to a COLUMN (`has_elevator`, `has_garden`, `pet_friendly`, `parking_type <> 'none'`, `furnished_status in ('furnished','partially_furnished')`) and six to an amenity slug, which the server folds into `amenities` — both mean ALL of them. The column wins wherever there is one, because a column is present on every row while a slug is free text a listing carries or does not. An unrecognised value is dropped rather than narrowing the search to nothing |
