 ...
 *
 * Measured across the Oxy repos in August 2026: root `AGENTS.md` files had grown
 * to 478 KB (Mercaria), 59 KB (Bloom) and 52 KB (Homiio), mostly by accretion —
 * a subsystem walkthrough or per-issue design note appended by the branch that
 * shipped it, often beside a line already pointing at the doc that says the same
 * thing.
 *
 * Nothing was wrong with any single one of those diffs. Each added true,
 * carefully-written prose about work that had just landed, reviewed as part of a
 * PR whose subject was the feature. The growth is invisible per-commit and only
 * exists in the sum, which is exactly the shape a gate catches and a convention
 * does not.
 *
 ...
 *      what `docs/` is for. This is the actual mechanism of that growth — it
